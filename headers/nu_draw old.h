...
struct Vector Border_Rect_Vertices;
struct Vector Border_Rect_Indices;
...
)
{
    ...
    Generate_Corner_Segment(&vertices[0], &indices[0], tl_a, PI, 1.5f * PI, node->border_radius_tl, node->border_left, node->border_top, node->width, node->height, border_r_fl, border_g_fl, border_b_fl, bg_r_fl, bg_g_fl, bg_b_fl, tl_pts, vertex_offset, index_count, 0, node->hide_background);
    if (node->hide_background) {
        vertex_offset += 2 * tl_pts;                   // inner + outer border only
        index_count += (tl_pts - 1) * 6;             // border tris only
    } else {
        vertex_offset += 3 * tl_pts + 1;               // border + background verts
        index_count += (tl_pts - 1) * 6 + (tl_pts - 1) * 3;
    }
    int TR = vertex_offset;
    Generate_Corner_Segment(&vertices[0], &indices[0], tr_a, 1.5f * PI, 2.0f * PI, node->border_radius_tr, node->border_top, node->border_right, node->width, node->height, border_r_fl, border_g_fl, border_b_fl, bg_r_fl, bg_g_fl, bg_b_fl, tr_pts, vertex_offset, index_count, 1, node->hide_background);
    if (node->hide_background) {
        vertex_offset += 2 * tr_pts;
        index_count += (tr_pts - 1) * 6;
    } else {
        vertex_offset += 3 * tr_pts + 1;
        index_count += (tr_pts - 1) * 6 + (tr_pts - 1) * 3;
    }
    int BR = vertex_offset;
    Generate_Corner_Segment(&vertices[0], &indices[0], br_a, 0.0f, 0.5f * PI, node->border_radius_br, node->border_right, node->border_bottom, node->width, node->height, border_r_fl, border_g_fl, border_b_fl, bg_r_fl, bg_g_fl, bg_b_fl, br_pts, vertex_offset, index_count, 2, node->hide_background);
    if (node->hide_background) {
        vertex_offset += 2 * br_pts;
        index_count += (br_pts - 1) * 6;
    } else {
        vertex_offset += 3 * br_pts + 1;
        index_count += (br_pts - 1) * 6 + (br_pts - 1) * 3;
    }
    int BL = vertex_offset;
    Generate_Corner_Segment(&vertices[0], &indices[0], bl_a, 0.5f * PI, PI, node->border_radius_bl, node->border_bottom, node->border_left, node->width, node->height, border_r_fl, border_g_fl, border_b_fl, bg_r_fl, bg_g_fl, bg_b_fl, bl_pts, vertex_offset, index_count, 3, node->hide_background);
    if (node->hide_background) {
        index_count += (bl_pts - 1) * 6;
    } else {
