...
static void NU_Clear_Node_Sizes(struct NU_GUI* gui)
{
    // Don't skip the root
    struct Node* root = NU_Tree_Get(&gui->tree, 0, 0); 
...
static void NU_Calculate_Text_Min_Width(struct NU_GUI* gui, struct Node* node)
{
    ...
    for (int i = 0; i <= len; i++) {
        ...
        if (c == ' ') {
            if (i > slice_start) {
                float bounds[4];
                nvgTextBounds(gui->vg_ctx, 0, 0, text + slice_start, text + i, bounds);
    ...
    if (max_word_width == 0.0f && len > 0) { // no spaces, measure whole string
        float bounds[4];
        nvgTextBounds(gui->vg_ctx, 0, 0, text, NULL, bounds);
...
static void NU_Calculate_Text_Fit_Size(struct NU_GUI* gui, struct Node* node)
{
    // Make sure the NanoVG context has the correct font/size set before measuring!
    int fontID = *(int*) Vector_Get(&gui->font_registry, 0);
    nvgFontFaceId(gui->vg_ctx, fontID);   
    nvgFontSize(gui->vg_ctx, 18);

    // Calculate text bounds
    float asc, desc, lh;
    float bounds[4];
    nvgTextMetrics(gui->vg_ctx, &asc, &desc, &lh);
    nvgTextBounds(gui->vg_ctx, 0, 0, node->text_content, NULL, bounds);
    float text_width = bounds[2] - bounds[0];
    float text_height = lh;
    
    NU_Calculate_Text_Min_Width(gui, node);
...
static void NU_Calculate_Text_Fit_Sizes(struct NU_GUI* gui)
{
    // Don't skip the root
    struct Node* root = NU_Tree_Get(&gui->tree, 0, 0); 
    if (root->text_content != NULL) {
        NU_Calculate_Text_Fit_Size(gui, root);
    }
    ...
    for (int l=0; l<=gui->deepest_layer; l++)
    {
        ...
        {   
            ...
            if (node->text_content != NULL) {
                NU_Calculate_Text_Fit_Size(gui, node);
            }
        }
    }
}

static void NU_Calculate_Fit_Size_Widths(struct NU_GUI* gui)
{
...
static void NU_Calculate_Fit_Size_Heights(struct NU_GUI* gui)
{
    if (gui->deepest_layer == 0) return;
...
static void NU_Grow_Shrink_Widths(struct NU_GUI* gui)
{
    for (uint16_t l=0; l<=gui->deepest_layer; l++)
    {
...
static void NU_Grow_Shrink_Heights(struct NU_GUI* gui)
{
    for (uint16_t l=0; l<=gui->deepest_layer; l++)
    {
        NU_Layer* child_layer = &gui->tree.layers[l+1];
...
static void NU_Calculate_Text_Wrap_Height(struct NU_GUI* gui, struct Node* node)
{
    // Configure font
    int fontID = *(int*) Vector_Get(&gui->font_registry, 0);
    nvgFontFaceId(gui->vg_ctx, fontID);   
    ...
    float bounds[4];
    nvgTextBoxBounds(gui->vg_ctx, 0, 0, inner_width, node->text_content, NULL, bounds);
...
static void NU_Calculate_Text_Wrap_Heights(struct NU_GUI* gui)
{
    // Don't skip the root
    struct Node* root = NU_Tree_Get(&gui->tree, 0, 0); 
    if (root->text_content != NULL) {
        NU_Calculate_Text_Wrap_Height(gui, root);
    }
...
void NU_Mouse_Hover(struct NU_GUI* gui)
{   
    // --------------------------------------
    ...
    if (gui->hovered_window == NULL) return;
    ...
    // ----------------------------------------------------
    for (int i=0; i<gui->window_nodes.size; i++)
    {
        struct Node* window_node = *(struct Node**) Vector_Get(&gui->window_nodes, i);
        if (window_node->window == gui->hovered_window) {
    ...
    SDL_GetWindowPosition(gui->hovered_window, &window_x, &window_y);
    ...
    while (stack.size > 0 && !break_loop) 
    {
        ...
        if (current_node->window != gui->hovered_window) break;
        stack.size -= 1;
...
void NU_Draw_Node_Text(struct NU_GUI* gui, struct Node* node, NVGcontext* vg)
{
    // Setup font
    int fontID = *(int*) Vector_Get(&gui->font_registry, 0);
...
void NU_Draw_Nodes(struct NU_GUI* gui)
{
    struct Vector window_nodes_list[MAX_TREE_DEPTH];
    for (uint32_t i=0; i<gui->windows.size; i++) {
    ...
    for (int l=0; l<=gui->deepest_layer; l++)
    {
        ...
        {   
            ...
            for (uint32_t i=0; i<gui->windows.size; i++)
            {
                SDL_Window* window = *(SDL_Window**) Vector_Get(&gui->windows, i);
    ...
    for (uint32_t i=0; i<gui->windows.size; i++)
    {
        ...
        nvgBeginFrame(gui->vg_ctx, w, h, 1.0f);
        ...
        for (int n=0; n<window_nodes_list[i].size; n++)
        {
            ...
            NU_Draw_Node(node, gui->vg_ctx, (float)w, (float)h);

            if (node->text_content != NULL)
            {
                NU_Draw_Node_Text(gui, node, gui->vg_ctx);
            }
        }

        // Render to window
        nvgEndFrame(gui->vg_ctx);
...
void NU_Calculate(struct NU_GUI* gui)
{
    ...
    NU_Grow_Shrink_Widths(gui);
    NU_Calculate_Text_Wrap_Heights(gui);
