    ...
    int corner_index)
{
    ...
    // --- Create inner background vertex ---
    static const float sign_lut_x[4] = { +1.0f, -1.0f, -1.0f, +1.0f };
    static const float sign_lut_y[4] = { +1.0f, +1.0f, -1.0f, -1.0f };
    ...
    float screen_width, float screen_height)
{
    ...
    // --- Top side quad ---
    indices[index_count++] = TL + tl_pts - 1;        // Last outer vertex of TL
    indices[index_count++] = TL + 2 * tl_pts - 1;    // Last inner vertex of TL
    indices[index_count++] = TR;                               // First outer vertex of TR
    indices[index_count++] = TL + 2 * tl_pts - 1;    // Last inner vertex of TL
    indices[index_count++] = TR + tr_pts;            // First inner vertex of TR
    indices[index_count++] = TR;                               // First outer vertex of TR

    // --- Right side quad ---
    indices[index_count++] = TR + tr_pts - 1;       // Last outer vertex of TR
    indices[index_count++] = TR + 2 * tr_pts - 1;   // Last inner vertex of TR
    indices[index_count++] = BR;                              // First outer vertex of BR
    indices[index_count++] = TR + 2 * tr_pts - 1;   // Last inner vertex of TR
    indices[index_count++] = BR + br_pts;           // First inner vertex of BR
    indices[index_count++] = BR;                              // First outer vertex of BR

    // --- Bottom side quad ---
    indices[index_count++] = BR + br_pts - 1;       // Last outer vertex of BR
    indices[index_count++] = BR + 2 * br_pts - 1;   // Last inner vertex of BR
    indices[index_count++] = BL;                              // First outer vertex of BL
    indices[index_count++] = BR + 2 * br_pts - 1;   // Last inner vertex of BR
    indices[index_count++] = BL + bl_pts;           // First inner vertex of BL
    indices[index_count++] = BL;                              // First outer vertex of BL

    // --- Left side quad ---
    indices[index_count++] = BL + bl_pts - 1;       // Last outer vertex of BL
    indices[index_count++] = BL + 2 * bl_pts - 1;   // Last inner vertex of BL
    indices[index_count++] = TL;                              // First outer vertex of TL
    indices[index_count++] = BL + 2 * bl_pts - 1;   // Last inner vertex of BL
    indices[index_count++] = TL + tl_pts;           // First inner vertex of TL
    indices[index_count++] = TL;                              // First outer vertex of TL    
    ...
    indices[index_count++] = TL_bg_connector - 1; // last top left background corner vertex
    indices[index_count++] = TR + tr_pts * 2;     // first top right background corner vertex
    indices[index_count++] = TL_bg_connector;
    ...
    indices[index_count++] = TR_bg_connector - 1; // last top right background corner vertex
    indices[index_count++] = BR + br_pts * 2;     // first bottom right corner vertex
    indices[index_count++] = TR_bg_connector;
    ...
    indices[index_count++] = BR_bg_connector - 1; // last bottom right background corner vertex
    indices[index_count++] = BL + bl_pts * 2;     // first bottom left background corner vertex
    indices[index_count++] = BR_bg_connector;
    ...
    indices[index_count++] = BL_bg_connector - 1; // last bottom left background corner vertex
    indices[index_count++] = TL + tl_pts * 2;     // first top left background corner vertex
    indices[index_count++] = BL_bg_connector;
